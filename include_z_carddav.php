            ...
            $unique_etags = array();
              ...
              foreach ($xml->response as $response) {
                if (isset($response->propstat)) {
					...
					// First check if response is adressbook
					
					if (isset($response->propstat->prop->resourcetype->addressbook)) {
                        // It's an addressbook
					...
					// If not the do vcard stuff
					
                    else if ((strlen($this->url_vcard_extension) > 0 && preg_match('/'.$this->url_vcard_extension.'/', $response->href) &&
